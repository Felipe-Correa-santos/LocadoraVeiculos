...
create database Locadora_de_Veículos;
show databases;
use Locadora_de_Veículos;

-- criando tabelas  e colocando os valores --
...
create table Locacao(
    ...
    constraint FKPagamento Foreign Key (ID_Pagamento) references Pagamento(ID_Pagamento),
	DataInicio Date,
    DataFim Date
);

insert into Locacao (ID_Locacao, ID_Cliente, ID_Pagamento, DataInicio, DataFim) values
...
create table LocacaoVeiculo( 
	ID_Locacao int,
	ID_Veiculo int,
	Primary Key (ID_Locacao, ID_Veiculo),
...
create table Manutencao(
    ...
    ID_Veiculo int,
	constraint FKVeiculo Foreign Key (ID_Veiculo) references Veiculo(ID_Veiculo),
	Descricao Varchar (100),
    DataManutencao Date,
    Custo Decimal (7,2)
);

insert into Manutencao (ID_Manutencao, ID_Veiculo, Descricao, DataManutencao, Custo) values
...
/*verificando se as tabelas foram criadas com sucesso e
conferindo se os dados estao corretos subistituindo o nome no comando da linha 172
para a tabela que desejo verificar os valores e dados
*/ 
 ...
 select ValorTotal from Pagamento
 where Estado = 'pago';

SELECT 
    Veiculo.Modelo,
    Veiculo.Marca,
    COUNT(LocacaoVeiculo.ID_Veiculo) AS Quantidade_Locacoes
FROM LocacaoVeiculo
JOIN Veiculo 
    ON LocacaoVeiculo.ID_Veiculo = Veiculo.ID_Veiculo
GROUP BY 
    Veiculo.Modelo,
    Veiculo.Marca
ORDER BY 
...
SELECT
    ...
    P.ValorTotal AS Valor_Pendente
FROM
    Cliente C
JOIN
    Locacao L ON C.ID_Cliente = L.ID_Cliente
JOIN
    Pagamento P ON L.ID_Pagamento = P.ID_Pagamento
WHERE
    P.Estado = 'Pendente'
ORDER BY
    C.Nome ASC;
